        ...
        if selected_piece is None:
            ...
            for p in pieces_array:
                if p.x == grid_x and p.y == grid_y:
                    ...
                    chess_piece.redraw_pieces_on_board_with_green_highlight(pieces_array, board, screen, (grid_x, grid_y))
                    break
...
def remove_piece_at_position(pieces, grid_x, grid_y, new_piece):
    updated_pieces = [p for p in pieces if not (p.x == grid_x and p.y == grid_y)]
...
def is_valid_move(piece, target_x, target_y, pieces):
    ...
    if piece.type == "pawn":
        ...
        for p in pieces:
            if (p.x == target_x and p.y < max(piece.y, target_y) and p.y > min(piece.y, target_y)) or (p.y == target_y and p.x < max(piece.x, target_x) and p.x > min(piece.x, target_x)):
                return False
            if abs(p.x - piece.x) == abs(p.y - piece.y) and p.x < max(piece.x, target_x) and p.x > min(piece.x, target_x) and p.y < max(piece.y, target_y) and p.y > min(piece.y, target_y):
                return False
        return True
